...
function import_itis_v1 {
   ...
   INSERT INTO "ZSPEC" (ZNR,ZSCI_NAME,X_KINGDOM,ZAUTHOR) select tsn,complete_name,kingdom_id,taxon_author_id from taxonomic_units as t1 WHERE (t1.unaccept_reason in ('junior homonym','invalidly published, nomen nudum','original name/combination','pro parte','horticultural','unnecessary replacement','orthographic variant (misspelling)','subsequent name/combination','junior synonym','nomen dubium','unavailable, incorrect orig. spelling','invalidly published, other','rejected name','nomen oblitum','misapplied','unjustified emendation','homonym & junior synonym','superfluous renaming (illegitimate)','synonym','homonym (illegitimate)') or t1.unaccept_reason is NULL);
...
#break 2> /dev/null
   ...
   psql $CONF[BACKBONE] $CONF[USERNAMEPOSTGRES] < GERMANSL.tmp.sql
